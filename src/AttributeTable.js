  ...
  useEffect(() => {
    const fetchAttributeData = async () => {
      if (attributeTable.loading && attributeTable.layerName) {
        try {
          const wfsUrl = `http://59.92.124.41:8086/geoserver/wfs?service=WFS&version=1.1.0&request=GetFeature&typeName=${attributeTable.layerName}&outputFormat=application/json`;
