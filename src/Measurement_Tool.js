...
const MeasurementTool = ({ map, draw, setDraw, setShowModal }) => {
  ...
  useEffect(() => {
    if (draw) {
      console.log("Draw interaction is active", draw);
    } else {
      console.log("Draw interaction is not active", draw);
    }
  }, [draw]);
    ...
    modifyInteraction.current.on("modifystart", (event) => {
      ...
      if (feature) {
        ...
        featureChangeListener.current = () => {
          ...
          console.log('This is current measurement');
  ...
  const startMeasurement = (type) => {
    ...
    console.log(`draw has been set to ${draw}`);
